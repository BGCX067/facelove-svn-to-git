...
int KeyWrap::wcscasecmp(const wchar_t* cs,const wchar_t * ct)
{
 ...
 void KeyWrap::Towlower(wchar_t* str)
 {	
	 do 
	 {
		 towlower(*str);
...
bool  KeyWrap::SetInputDevice(map<wstring,MMAudioDev>& m_devs,int nType,int nState,wchar_t* szShengKa)
{
	...
	for (map<wstring,MMAudioDev>::iterator it=m_devs.begin();it!=m_devs.end();it++)
	{
	    ...
	    if (szShengKa ==NULL || wcscmp(dev.szDeviceName,szShengKa)!=NULL )
	    {
			if(nType==0)
			{
				if( wcscmp(dev.szEndPointName,_T("mic"))!=NULL || wcscmp(dev.szEndPointName,_T("麦克风"))!=NULL) 
					bFind=true;
			}
			else if(nType==1)
			{
				
				bool temp =(wcsstr(dev.szEndPointName,_T("mix"))!=NULL);
				temp =(wcsstr(dev.szEndPointName,_T("sum"))!=NULL);
				temp =(wcsstr(dev.szEndPointName,_T("混"))!=NULL);
				temp =(wcsstr(dev.szEndPointName,_T("立体"))!=NULL);
				temp =( wcsstr(dev.szEndPointName,_T("stereo")));
				temp =(wcsstr(dev.szEndPointName,_T("总和"))!=NULL);
			...
			if(nType==2)
			{
				if(wcscmp(dev.szEndPointName,_T("line"))!=NULL||wcscmp(dev.szEndPointName,_T("线路"))!=NULL) 
					bFind=true;
...
bool KeyWrap::WriteImputDeviceState(HKEY hKey,wchar_t* lpSubKey, DWORD dwVa)
{
   ...
   // lReturn =RegSetValue(hKey,lpSubKey,REG_DWORD,(LPCWSTR ) &dwVa,sizeof(DWORD));
	//RegCreateKeyEx(hKey,lpSubKey,0L,REG_DWORD,REG_OPTION_NON_VOLATILE, KEY_WRITE|KEY_READ,NULL,KEY_ALL_ACCESS,)
...
{
	long lReturn=RegSetValueEx(hKey,lpSubKey,0L,REG_BINARY,(const BYTE *) lpSubKey,5*strlen((const char*)lpSubKeyValue)+1);
...
=======
 ...
 bool KeyWrap::EnableDebugPrivilege()
 {
	 HANDLE hToken; 
 ...
 bool KeyWrap::UpPrivilege()
 {
	 BOOL fIsRunAsAdmin = TRUE;
