...
void MixerWrap::MicphoneMut(bool bIsMut)
{
	...
	for(int i=0;i<mixerNum;i++)
	{
		...
		for ( u = 0; u < MixerLine.cControls; u++)
		{				
			if (wcscmp(paMixerControls[u].szName,_T("麦克风静音"))==0 || wcscmp(paMixerControls[u].szName,_T("静音"))==0)
			{
				ifFind =true;
				break;
			}
	...
	for(int i=0;i<mixerNum;i++)
	{
			...
			//如一个audio line目标为“音量控制”，另一个audio line目标为“录音控制”。
		{
			...
			for ( int count = 0; count < dwConnections; count++ )
			{
				...
				if (wcscmp(MixerLine.szName,_T("立体声混音"))==0/*MIXERLINE_COMPONENTTYPE_SRC_COMPACTDISC == MixerLine.dwComponentType &&*/ )
				{
					//如果是立体声混音选项，则跳出	
		...
		{		
			...
			if (wcscmp(paMixerControls[u].szName,_T("立体声混音静音"))==0 || 
				wcscmp(paMixerControls[u].szName,_T("静音"))==0 ||
				wcscmp(paMixerControls[u].szName,_T("立体声混音"))==0 )
			{
				ifFind =true;
				break;
			}
...
bool MixerWrap::MicphoneBoost_W7(bool bIsBoost)
{
	HRESULT hr=NULL; 
	CoInitialize(NULL);
	...
	if (bIsBoost)
	{
		...
		pIaudioVolumeLevel->SetLevel(0,fStepDb,NULL);//0-30
	}
	else
	{
		pIaudioVolumeLevel->SetLevel(0,0,NULL);//0-30
...
void MixerWrap::StereoMixVolume(int nVolume)
{
	...
	for(int i=0;i<mixerNum;i++)
	{
		...
		{		
			//_tprintf(_T("MixerLine.cControls:%d,MpaMixerControls[u].szName:%s,\r\n"),MixerLine.cControls,paMixerControls[u].szName);			
			if (wcscmp(paMixerControls[u].szName,_T("立体声混音"))==0 || wcscmp(paMixerControls[u].szName,_T("立体声混音音量"))==0 )
			{
				ifFind =true;
				break;
			}
...
void MixerWrap::OutPutMicphoneVolume(int nVolume)
{
	...
	for(int i=0;i<mixerNum;i++)
	{
		...
		for ( u = 0; u < MixerLine.cControls; u++)
		{				
			if (wcscmp(paMixerControls[u].szName,_T("麦克风音量"))==0 || wcscmp(paMixerControls[u].szName,_T("静音"))==0)
			{
				ifFind =true;
				break;
			}
...
void MixerWrap::InPutMicphoneVolume(int nVolume)
{
	...
	for(int i=1;i<mixerNum;i++)
	{
		...
		for ( u = 0; u < MixerLine.cControls; u++)
		{				
			if (wcscmp(paMixerControls[u].szName,_T("麦克风输入音量"))==0 || wcscmp(paMixerControls[u].szName,_T("麦克风音量"))==0)
			{
				ifFind =true;
				break;
			}
...
int   MixerWrap::SelectRecordInByMicphone()
{
	try{ 
		HMIXER     m_hmx; //混音器设备句柄
		MMRESULT      mmr = mixerOpen(&m_hmx, 1, 0, 0L, MIXER_OBJECTF_MIXER);
		...
		{ 
			...
			{ 
				...
				if   (wcsstr(plisttext[j].szName,_T("麦克风"))!=NULL ) 
				{ 
					plistbool[j].fValue   =   plistbool[j+   cChannels   -   1].fValue   =   1; 
				} 
				else   if   (bOneItemOnly) 
...
int   MixerWrap::SelectRecordInByStereo()
{
	try{ 
		HMIXER     m_hmx; //混音器设备句柄
		MMRESULT      mmr = mixerOpen(&m_hmx, 1, 0, 0L, MIXER_OBJECTF_MIXER);
		...
		{ 
			...
			{ 
				...
				if   (wcsstr(plisttext[j].szName,_T("立体声混音"))!=NULL ) 
				{ 
					plistbool[j].fValue   =   plistbool[j+   cChannels   -   1].fValue   =   1; 
				} 
				else   if   (bOneItemOnly) 
...
bool   MixerWrap::IsInputSingleSelect()
{
	try{ 
		HMIXER m_hmx; //混音器设备句柄
		MMRESULT mmr = mixerOpen(&m_hmx, 1, 0, 0L, MIXER_OBJECTF_MIXER);
